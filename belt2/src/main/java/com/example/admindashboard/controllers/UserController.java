...
public class UserController {
	...
	public String register(@Valid @ModelAttribute("user") User user,BindingResult res,Model model, @ModelAttribute("role") Role role){
		userValidator.validate(user,res);
		System.out.println(roleService.findByName("ROLE_SUPERADMIN").getUsers().size());
		if(res.hasErrors()){return "login_register";}

		else if(roleService.findByName("ROLE_SUPERADMIN").getUsers().size() < 1){ 
			userService.create(new String[]{"ROLE_USER","ROLE_SUPERADMIN"}, user);
